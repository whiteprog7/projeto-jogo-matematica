...
writeFileSync(path.join(temp,'server.mjs'),compile(readFileSync('lib/server.ts','utf8').replace("import {env} from 'cloudflare:workers';","const env={get DB(){return globalThis.testDB},ADMIN_EMAIL:'owner@example.test'};")));
let src=readFileSync('app/api/game/route.ts','utf8').replace("import {getChatGPTUser} from '../../chatgpt-auth';","const getChatGPTUser=async()=>globalThis.testUser;").replace("from '@/lib/server'","from './server.mjs'").replace("from '@/lib/content'","from './content.mjs'");
writeFileSync(path.join(temp,'route.mjs'),compile(src));
writeFileSync(path.join(temp,'admin.mjs'),compile(readFileSync('app/api/admin/route.ts','utf8').replace("import {getChatGPTUser} from '../../chatgpt-auth';","const getChatGPTUser=async()=>globalThis.testUser;").replace("from '@/lib/server'","from './server.mjs'")));
...
identity('alice');assert.equal((await post({action:'profile',name:'Alice'})).status,200);assert.equal((await post({action:'class',name:'6º A'})).status,403);assert.equal((await get('?view=teacher')).status,403);assert.equal((await post({action:'gear',gear:3})).status,403);assert.equal((await post({action:'start',region:0})).status,403);assert.equal((await approve('alice')).status,200);assert.equal((await admin.GET()).status,403);
identity('teacher');await post({action:'profile',name:'Professor',requestedRole:'teacher'});assert.equal((await get('?view=teacher')).status,403);await approve('teacher','teacher');const c=await post({action:'class',name:'6º A'});assert.ok(c.code);
