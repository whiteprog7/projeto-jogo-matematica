...
export async function GET(){try{const user=await getCurrentUser();return reply({account:user?await accountInfo(user):null})}catch{return reply({error:'Não foi possível consultar sua conta. Tente novamente.'},503)}}
export async function POST(request:Request){try{
 ...
 if(!login||login.length>80||!['student','teacher','admin'].includes(b.role))return reply({error:'Informe seu ID, senha e tipo de conta.'},400);
 if(await limited('login:'+login,12,15*60000))return reply({error:'Muitas tentativas para esta conta. Aguarde 15 minutos.'},429);
 const adminLogin=login==='ADM123'||login==='ADM-0001';
 const a=adminLogin?null:await db().prepare('SELECT * FROM accounts WHERE login_id=?').bind(login).first<any>();
 const stored=adminLogin?config().ADMIN_PASSWORD_HASH:a?.password_hash;
 ...
 return reply({account},200,await issueSession(userId));
 }
 ...
 return reply({account},201,await issueSession(userId));
