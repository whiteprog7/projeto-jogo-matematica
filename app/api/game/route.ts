...
export async function GET(request:Request){try{
 ...
 if(view==='me')return reply({profile:p,stats:await stats(user.userId),admin,approved:admin||p?.status==='approved',teacher:admin||await teacherAllowed(user.userId),identity:user.userId,class:p?.class_id?await db().prepare('SELECT name FROM classes WHERE id=?').bind(p.class_id).first():null});
...
export async function POST(request:Request){try{
 ...
 await db().prepare("INSERT INTO profiles(id,name,email,requested_role,status) VALUES(?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET name=excluded.name,email=excluded.email,requested_role=excluded.requested_role").bind(id,name,user.email,requested,admin?'approved':'pending').run();return reply({ok:true})}
